...
add_action('after_setup_theme', function(){ // hook para que se ejecute después de que el tema esté configurado
    if(isset($_POST['nonce'])  and $_POST['action']=='pasarela-edit'){
        ...
        wp_safe_redirect($_POST['return']);exit; // redirección segura después de la actualización
    }
});

 //ajax
 if(!function_exists('itc_tienda_carro_pasarelas_ajax')){
    function itc_tienda_carro_pasarelas_ajax(){
        global $wpdb;
       
        $datos=$wpdb->get_results("select * from {$wpdb->prefix}itc_tienda_carro_pasarelas where id='".sanitize_text_field($_POST['id'])."';", ARRAY_A); 
       
        ?>
        <div class="row">
            <form action="" method="POST" name="itc_tienda_form_pasarela">
                <div class="mb-3">
    ...
    add_action('wp_ajax_itc_tienda_pasarelas_ajax','itc_tienda_carro_pasarelas_ajax');
}