...
add_action( 'after_setup_theme', function(){
    if(isset($_POST['nonce']) and $_POST['action'] == 'registro-in' ){
        $userdata=[
            'user_login'=>sanitize_text_field($_POST['correo']), // sanitize_text_field es una función de WordPress que limpia los datos de entrada
            'user_pass'=>sanitize_text_field($_POST['password']),
            'user_email'=>sanitize_text_field($_POST['correo']),
            'first_name'=>sanitize_text_field($_POST['nombre']),
            'last_name'=>sanitize_text_field($_POST['apellido']),
...
if(!function_exists('itc_tienda_registro_codigo_corto_display')){
    function itc_tienda_registro_codigo_corto_display($argumentos, $content=""){
        ...
        if(is_user_logged_in()){
         ...
         $html.='<div class="container"><form action="" method="POST" name="itc_tienda_registro_form">';
