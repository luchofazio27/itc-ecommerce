...
if(!function_exists('tamila_tienda_variables_globales_post')){
    function tamila_tienda_variables_globales_post() {
        if(isset($_POST['nonce']) and $_POST['action']=='variables-globales-edit'){
            global $wpdb;
            $wpdb->query("update {$wpdb->prefix}tamila_tienda_variables_globales 
                    set  
    ...
    add_action( 'after_setup_theme', 'tamila_tienda_variables_globales_post' );

//ajax
if(!function_exists('tamila_tienda_carro_variables_globales_ajax')){
    function tamila_tienda_carro_variables_globales_ajax(){
        global $wpdb;
        
        $datos=$wpdb->get_results("select * from {$wpdb->prefix}tamila_tienda_variables_globales where id='".sanitize_text_field($_POST['id'])."';", ARRAY_A); 
        ?>
        <div class="row">
            <form action="" method="POST" name="tamila_tienda_form_variables_globales">
    ...
    add_action('wp_ajax_tamila_tienda_variables_globales_ajax','tamila_tienda_carro_variables_globales_ajax');
}