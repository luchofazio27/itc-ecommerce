...
$fotos = $wpdb->get_results(
    "select * from {$wpdb->prefix}tamila_tienda_productos_galeria where post_id='" .
        $post->ID .
        "' order by id desc;",
    ARRAY_A
); 
$carro = $wpdb->get_results(
    "
    select 
    {$wpdb->prefix}tamila_tienda_carro_detalle.id
    from 
                ...
                <div class="card">
                    ...
                    <nav aria-label="breadcrumb">
                        ...
                        <div class="col d-grid">
                            ...
                            if(sizeof($carro)==0){
                                ?>
                            <a href="javascript:void(0);" class="btn btn-outline-danger btn-lg" onclick="tamila_tienda_comprar('<?php the_ID();?>' , '<?php echo admin_url("admin-ajax.php");?>', '<?php echo wp_create_nonce("seg"); ?>', '<?php echo get_site_url();?>/checkout', '1');" title="Comprar">
                        ...
                        if(sizeof($carro)==0){
                            ?>
                            <a href="javascript:void(0);" class="btn btn-outline-success btn-lg" onclick="tamila_tienda_comprar('<?php the_ID();?>' , '<?php echo admin_url("admin-ajax.php");?>', '<?php echo wp_create_nonce("seg"); ?>', '<?php echo get_permalink();?>', '1');" title="Añadir al carrito">
                            <i class="fa fa-fw fa-cart-arrow-down"></i> Añadir al carrito
                            </a>
                            <?php
                        }else{
                            ?>
                            <a href="javascript:void(0);" class="btn btn-outline-warning btn-lg" onclick="tamila_tienda_comprar('<?php the_ID();?>' , '<?php echo admin_url("admin-ajax.php");?>', '<?php echo wp_create_nonce("seg"); ?>', '<?php echo get_permalink();?>', '2');" title="Quitar del carrito">
